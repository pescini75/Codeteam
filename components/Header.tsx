...
const navItems: NavItem[] = [
  ...
  { label: 'La nostra filosofia', href: '#filosofia' },
...
const Header: React.FC<HeaderProps> = ({ onNavigate }) => {
  ...
  return (
    <header
      role="banner"
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled || isMobileMenuOpen ? 'bg-slate-950/90 backdrop-blur-lg shadow-lg border-b border-slate-800' : 'bg-transparent'
        }`}
    >
      ...
      <div
        className={`lg:hidden bg-slate-950/98 backdrop-blur-3xl border-t border-slate-800 overflow-hidden relative transition-all duration-500 ease-in-out ${isMobileMenuOpen ? 'max-h-[60vh] opacity-100' : 'max-h-0 opacity-0'
          }`}
      >
        ...
        <nav aria-label="Navigazione Mobile" className="relative z-10 px-8 pt-6 pb-8 space-y-0.5">
          {navItems.map((item, idx) => (
            <a
              ...
              className={`block px-4 py-2 rounded-xl text-sm font-bold text-slate-400 hover:text-brand-500 hover:bg-slate-900/40 border border-transparent hover:border-brand-500/20 transition-all transform tracking-widest uppercase ${isMobileMenuOpen ? 'translate-x-0 opacity-100' : '-translate-x-4 opacity-0'
                }`}
            >
              {item.label}
            </a>
          ...
          <div
            className={`pt-5 transition-all duration-500 delay-300 transform ${isMobileMenuOpen ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'
              }`}
          >
            <a
              href="#contact"
